  ...
  <section class="login">
    <section class="entry" v-if="state === STATES.NEW_OR_LOGIN" :class="{'success':success}">
      ...
      <section class="body">
        ...
        <section v-if="!isNewScatter">
          <Input
            class="welcome-password"
            :focus="true"
            big="1"
            for-login="1"
            placeholder="Enter your password"
            type="password"
            :disabled="opening || isLockedOut"
            :loader-on-dynamic="opening && !success"
            :text="password"
            v-on:enter="unlock"
            v-on:dynamic="unlock"
            v-on:changed="x => password = x"
            :dynamic-button="badPassword ? 'icon-cancel' : success ? 'icon-check' : isLockedOut ? '' : 'icon-right-open-big'"
            :hide-dynamic-button="!password.length"
          />
        </section>
      </section>
...
const getLockout = () =>
  JSON.parse(
    window.localStorage.getItem("lockout") ||
      JSON.stringify({ tries: 0, stamp: 0 })
  );
const setLockout = () => {
...
export default {
  ...
  methods: {
    ...
    importKeypair() {
      this.stepForward();
      PopupService.push(
        Popup.importKeypair({ forSignup: true }, keypair => {
          this.stepForward();
        })
      );
    },
    ...
    ...mapActions([Actions.SET_SEED, Actions.LOAD_SCATTER, Actions.SET_SCATTER])
  },
  watch: {
    ["password"]() {
      ...
      .details {
        ...
        .logo {
          ...
          text-shadow: 0 0 90px rgba(255, 255, 255, 0.6),
            0 1px 2px rgba(0, 0, 0, 0.2), 0 3px 7px rgba(0, 0, 0, 0.2);
          color: $white;
